...
const SEGMENT_BATCH_SIZE = process.env.SEGMENT_BATCH_SIZE || 2; // Control the number of active segments
...
const WORKER_SELECTION_STRATEGY =
  process.env.WORKER_SELECTION_STRATEGY || WORKER_SELECTION_STRATEGY_LOAD_RANK;
...
class WorkerSet {
  ...
  updateStats(socketId, stats) {
    ...
    if (worker) {
      ...
      console.warn(
        `Received stats from unregistered worker on socket ${socketId}`
      );
    }
  }
  ...
  getWorkerSelectionStrategy() {
    console.debug(
      `Using Worker Selection Strategy: ${WORKER_SELECTION_STRATEGY}`
    );
  ...
  loadBasedSelector() {
    ...
    if (currentWorkers.length > 0) {
      return currentWorkers.reduce((a, b) =>
        a.stats.cpu < b.stats.cpu ? a : b
      );
    } else {
      return null;
    }
  ...
  taskLoadBasedSelector() {
    ...
    if (currentWorkers.length > 0) {
      return currentWorkers.reduce((a, b) =>
        a.stats.tasks < b.stats.tasks ? a : b
      );
    } else {
      return null;
    }
...
class JobPosterSet {
  ...
  findBySocketId(id) {
    for (const jobPoster of this.items.values()) {
      if (jobPoster.socketId === id) return jobPoster;
    }
    return null;
  }
}
...
class WorkQueue {
  ...
  update(taskUpdate) {
    console.log(
      `Received update for task ${taskUpdate.taskId}, status: ${taskUpdate.taskStatus}`
    );
  ...
  killWorkerTasks(workerId) {
    ...
    for (const task of Array.from(this.tasks.values()).filter(
      (x) => x.workerId === workerId
    )) {
      this.update(
        new TaskUpdate(task.id, "done", false, "Worker disconnected")
      );
    }
  }
...
module.exports.init = (server) => {
  ...
  function runTask(task) {
    ...
    if (worker) {
      ...
      workQueue.update(
        new TaskUpdate(task.id, "done", false, "No worker available")
      );
    }
  }
  ...
  function jobComplete(job) {
    console.log(
      `Job ${job.id} complete, tasks: ${job.tasks.length}, result: ${job.result}`
    );
  ...
  function jobPosterDisconnectionHandler(socket) {
    ...
    if (jobPoster) {
      ...
      workQueue.kill((v) => {
        return v.jobPosterId === jobPoster.id;
      });
    }
  }
...
function multiWorkerTaskBuilder(job) {
  ...
  // Set segment duration directly to minSegDuration
  let segmentDuration = minSegDuration; // seconds
  const SEGMENTS_PER_NODE = SEGMENT_BATCH_SIZE;
